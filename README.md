...
* `pool_size`, optional, number of connections to maintain in each pool, see `Finch` docs for details, default to 100.
* `pool_count`, optional, number of pools to start, see `Finch` docs for details, default to 1.
...
Add the following environment variables to `config/test.exs`, then run
`mix test` in the root of this repo.
